...
** 简介
   ...
   - 可以在保留空白与忽略空白二者之间切换
   - 能够静默跳过任意位置的 XML 声明头、DTD 声明与处理指令
   - 支持标签内的 XML 注释

** 安装

   （注：需要预先安装 [[https://github.com/cisco/ChezScheme][Chez Scheme (v10.4.1)]]。）

   chez-xml 是一个单文件库，
   ~./src/xml.sls~ 为源代码文件，
   可直接加载，也可编译为 ~xml.so~ 以加速载入。
...
*** 基础步骤
    ...
    #+begin_src shell
      ...
      git clone https://github.com/Yunoinsky/chez-xml # 克隆仓库
      cd chez-xml
      echo '(compile-file "src/xml.sls")' | scheme -q  # 可选：预编译
...
*** 全局安装
    ...
    为了在其他项目中使用 chez-xml，
    我们可以将 ~xml.sls~ 拷贝到对应项目中，
    也可以选择编译后全局安装。下面提供一种全局安装的方式

    1. 编译：~echo '(compile-file "src/xml.sls")' | scheme -q~
    2. 创建 ~/Library/chezscheme/ 目录
    3. 将 ~xml.so~ 拷贝到该目录
    4. 将 ~/Library/chezscheme/ 添加到环境变量 ~CHEZSCHEMELIBDIRS~ 中
    5. 此后 ~(import (xml))~ 可在任意项目中使用
...
*** 加载库
    ...
    #+begin_src scheme
      (load "./src/xml.sls") ;; 如果已全局安装，可省略加载
      (import (xml))
    #+end_src
